...
// 03/08/2022:		Set codepage 1252, fixed keyboard mappings for AGON, added cursorTab, VDP serial
// protocol 06/08/2022:		Added a custom font, fixed UART initialisation, flow control 10/08/2022:
// Improved keyboard mappings, added sprites, audio, new font
...
// 04/10/2022:		Version 1.01: Can now change keyboard layout, origin and sprites reset on mode
// change, available modes tweaked 23/10/2022:		Version 1.02: Bug fixes, cursor visibility and
// scrolling 15/02/2023:		Version 1.03: Improved mode, colour handling and international
// support
// 04/03/2023:					+ Added logical screen resolution, sendScreenPixel now sends palette
// index as well as RGB values
// 09/03/2023:					+ Keyboard now sends virtual key data, improved VDU 19 to handle
// COLOUR l,p as well as COLOUR l,r,g,b 15/03/2023:					+ Added terminal support for
// CP/M, RTC support for MOS 21/03/2023:				RC2 + Added keyboard repeat delay and rate,
// logical coords now selectable 22/03/2023:					+ VDP control codes now indexed from
// 0x80, added paged mode (VDU 14/VDU 15) 23/03/2023:					+ Added VDP_GP 26/03/2023:
// RC3 + Potential fixes for FabGL being overwhelmed by faster comms
...
// 17/04/2023:				RC5 + Moved wait_completion in vdu so that it only executes after
// graphical operations 18/04/2023:					+ Minor tweaks to wait completion logic
...
fabgl::VGABaseController*
    VGAController;  // Pointer to the current VGA controller class (one of the above)
...
void init_audio_channel(int channel) {
	xTaskCreatePinnedToCore(
	    ...
	    &channel,  // Parameters
	    PLAY_SOUND_PRIORITY,  // Priority, with 3 (configMAX_PRIORITIES - 1) being the highest, and
	                          // 0 being the lowest.
...
void sendKeyboardState() {
	...
	byte packet[] = {kbRepeatDelay & 0xFF, (kbRepeatDelay >> 8) & 0xFF, kbRepeatRate & 0xFF,
	                 (kbRepeatRate >> 8) & 0xFF, scrollLock | (capsLock << 1) | (numLock << 2)};
...
fabgl::VGABaseController* get_VGAController(int colours) {
	switch (colours) {
		case 2:
			return VGAController2.instance();
		case 4:
			return VGAController4.instance();
		case 8:
			return VGAController8.instance();
		case 16:
			return VGAController16.instance();
		case 64:
			return VGAController64.instance();
	}
	return nullptr;
}
...
void setPaletteItem(int l, RGB888 c) {
	if (l < VGAColourDepth) {
		switch (VGAColourDepth) {
			case 2:
				VGAController2.setPaletteItem(l, c);
				break;
			case 4:
				VGAController4.setPaletteItem(l, c);
				break;
			case 8:
				VGAController8.setPaletteItem(l, c);
				break;
			case 16:
				VGAController16.setPaletteItem(l, c);
				break;
		}
	}
}
...
int change_mode(int mode) {
	...
	debug_log("do_modeChange: canvas(%d,%d), scale(%f,%f)\n\r", Canvas->getWidth(),
	          Canvas->getHeight(), logicalScaleX, logicalScaleY);
	return 0;
}
...
void do_keyboard() {
	...
	if (kb->getNextVirtualKey(&item, 0)) {
		if (item.down) {
			...
			//
			modifiers = item.CTRL << 0 | item.SHIFT << 1 | item.LALT << 2 | item.RALT << 3 |
			            item.CAPSLOCK << 4 | item.NUMLOCK << 5 | item.SCROLLLOCK << 6 |
			            item.GUI << 7;
		}
...
// These can send responses back; the response contains a packet # that matches the VDU command mode
// byte
//
void vdu_sys_video() {
	int mode = readByte_t();
...
//
void vdu_sys_audio() {
	int channel = readByte_t();
	if (channel == -1)
		return;
	int waveform = readByte_t();
	if (waveform == -1)
		return;
	int volume = readByte_t();
	if (volume == -1)
		return;
	int frequency = readWord_t();
	if (frequency == -1)
		return;
	int duration = readWord_t();
	if (duration == -1)
		return;
...
void vdu_sys_keystate() {
	...
	if (d >= 250 && d <= 1000)
		kbRepeatDelay = (d / 250) * 250;  // In steps of 250ms
	if (r >= 33 && r <= 500)
		kbRepeatRate = r;
...
//
void vdu_sys_scroll() {
	int extent = readByte_t();
	if (extent == -1)
		return;  // Extent (0 = current text window, 1 = entire screen)
	int direction = readByte_t();
	if (direction == -1)
		return;  // Direction
...
void vdu_sys_sprites(void) {
	...
	switch (cmd) {
		...
		case 2: {  // Define bitmap in single color
			...
			//
			dataptr = (void*)heap_caps_malloc(sizeof(uint32_t) * width * height,
			...
			if (dataptr != NULL) {
				if (cmd == 1) {
					...
					for (n = 0; n < width * height; n++)
						((uint32_t*)bitmaps[current_bitmap].data)[n] = readLong_b();
					debug_log(
					    "vdu_sys_sprites: bitmap %d - data received - width %d, height %d\n\r",
					    current_bitmap, width, height);
				}
				if (cmd == 2) {
					color = readLong_b();
					//
					...
					//
					for (n = 0; n < width * height; n++)
						((uint32_t*)dataptr)[n] = color;
					debug_log(
					    "vdu_sys_sprites: bitmap %d - set to solid color - width %d, height %d\n\r",
					    current_bitmap, width, height);
				}
				...
				for (n = 0; n < width * height; n++)
					readLong_b();  // discard incoming data
				debug_log(
				    "vdu_sys_sprites: bitmap %d - data discarded, no memory available - width %d, "
				    "height %d\n\r",
				    current_bitmap, width, height);
			}
		} break;
		...
		case 3: {  // Draw bitmap to screen (x,y)
			 ...
			 * 2) Select the correct sprite ID (0-255). The GDU only accepts sequential sprite sets,
			 * starting from ID 0. All sprites must be adjacent to 0 3) Clear out any frames from
			 * previous program definitions 4) Add one or more frames to each sprite 5) Activate
			 * sprite to the GDU 6) Show sprites on screen / move them around as needed 7) Refresh
			 */
		...
		case 6: {  // Add frame to sprite
			...
			debug_log("vdu_sys_sprites: sprite %d - bitmap %d added as frame %d\n\r",
			          current_sprite, n, sprites[current_sprite].framesCount - 1);
		} break;
		...
		case 9: {  // Set previous frame on sprite
			...
			if (n)
				sprites[current_sprite].currentFrame = n - 1;  // previous frame
			else
				sprites[current_sprite].currentFrame =
				    sprites[current_sprite].framesCount - 1;  // last frame
		...
		case 10: {  // Set current frame id on sprite
			...
			if (b == -1)
				return;
			n = b;
			if (n < sprites[current_sprite].framesCount)
				sprites[current_sprite].currentFrame = n;
