...
int change_resolution(int colours, char *modeLine)
{
	...
	{			  // If controller is null, then an invalid # of colours was passed
	...
	{ // Is it a different controller?
		if (VGAController)
		{						  // If there is an existing controller running then
	...
	if (modeLine)
	{											// If modeLine is not a null pointer then
...
void vdu_palette()
{
	...
	{ // If it is a paletted video mode
		if (p == 255)
		{ // If p = 255, then use the RGB values
			col = RGB888(r, g, b);
		}
		else if (p < 64)
		{ // If p < 64, then look the value up in the colour lookup table
			col = colourLookup[p];
		}
		else
		{
...
void vdu_sys_sprites(void)
{
	...
	case 9:
	{ // Set previous frame on sprite
		n = sprites[current_sprite].currentFrame;
		if (n)
			sprites[current_sprite].currentFrame = n - 1; // previous frame
		else
			sprites[current_sprite].currentFrame = sprites[current_sprite].framesCount - 1; // last frame
		debug_log("vdu_sys_sprites: sprite %d previous frame\n\r", current_sprite);
	}
	break;

	case 10:
	{ // Set current frame id on sprite
		int b = readByte_t();
		if (b == -1)
			return;
		n = b;
	...
	case 13:
	{ // Move sprite to coordinate on screen
		int rx = readWord_t();
		if (rx == -1)
			return;
		x = rx;
	...
	case 14:
	{ // Move sprite by offset to current coordinate on screen
		int rx = readWord_t();
		if (rx == -1)
			return;
		x = rx;
